...
def plot_base_pattern_collection(pattern_df_smooth, savdir,fn ,colorful, windows_lens):
    ...
    for i in range(12):
        ...
        ax.plot(xaxs, df_smooth[0][header[i]], alpha=alphaA, color="red")
...
def evaluate(args):
    ...
    # print("6-----------> ",smooth_windows_len, " : " ,args.smooth_window_Hz," <:smooth_window_Hz<=>get_freq_delta(pattern_files[0]):> " ,get_freq_delta(pattern_files[0]))
    # Build the pattern
    pattern_df = None
    # colorful =["red","blue","pink","orange","yellow"]
    id = 0
    pdList = []
    for file in tqdm(pattern_files, desc="Calculate the normal pattern...", leave=True):
        ...
        if pattern_df is None:
            ...
            pattern_df += df
    # plot_base_pattern_collection(pdList, args.output_dir, "collectionBase","black",smooth_windows_len)
    # plt.savefig(os.path.join(args.output_dir, "collectionBase.png"))
    # plt.close()
    pattern_df /= len(pattern_files)
    # print("1    ------------> ",pattern_df," : " ,pattern_files)
    pdList.append(pattern_df)
    plot_base_pattern_collection(pdList, args.output_dir, "collectionBase","black")
    # Smooth the pattern
    pattern_df_smooth = pattern_df.rolling(window=smooth_windows_len, center=True).mean().dropna()

    # print("7    ------------> ",pattern_df_smooth.shape,pattern_df_smooth)
    # Draw the pattern
